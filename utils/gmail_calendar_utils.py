...
class GmailCalendarUtils:
    ...
    async def emails_to_calendar_events(
        ...
        date_from: Optional[str] = None,  # ISO date string (e.g., '2025-10-01')
        ...
        try:
            ...
            logger.info(f"Building Gmail query: {final_query}")
            logger.info(
                f"Starting emails_to_calendar_events: max={max_emails}, query={query}"
            )

            # Step 1: List emails
            emails_result = await GmailHelpers.list_messages(
                access_token=self.gmail_token, query=final_query, max_results=max_emails
            )
            ...
            for msg in messages:
                try:
                    ...
                    email_result = await GmailHelpers.get_message(
                        access_token=self.gmail_token, message_id=msg["id"]
                    )
    ...
    async def _create_event_from_email(
        self, subject: str, from_email: str, body: str, email_date: str
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            try:
                ...
                event_start = parsed_dt
            except Exception as parse_err:
                logger.warning(
                    f"Failed to parse email date '{email_date}': {parse_err}"
                )
            ...
            result = await GCalendarHelpers.create_event(
                ...
                summary=f"📧 {subject or 'Email Event'}",
                start_time=start_time,
                end_time=end_time,
                description=description,
                timezone=str(event_start.tzinfo) if event_start.tzinfo else "UTC",
            )
